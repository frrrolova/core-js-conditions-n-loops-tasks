 ...
 * @return {boolean} True if the queen can capture the king, false otherwise.Checks if a queen can capture a king in the next move on an 8x8 chessboard
 *
 * @example
...
function getBalanceIndex(arr) {
  ...
  for (let i = 0; i < len; i += 1) {
    ...
    rightsum = 0;
    for (let j = i + 1; j < len; j += 1) rightsum += arr[j];
...
function shuffleChar(str, iterations) {
  let localIter = iterations;
  let res = str;
  let left = res[0];
  let right = '';
  let count = 0;

  while (count < localIter) {
    if (count > 0 && res === str) {
      localIter = iterations % count;
      count = 0;
    }
